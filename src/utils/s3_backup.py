...
class S3BackupManager:
    ...
    def upload_database_backup(self, database_file_path: str) -> bool:
        ...
        try:
            ...
            # Загружаем файл в S3
            self.s3_client.upload_file(database_file_path, self.bucket_name, s3_key)
            ...
            self.s3_client.upload_file(database_file_path, self.bucket_name, latest_key)
    ...
    def download_latest_backup(self, download_path: str) -> dict:
        ...
        try:
            ...
            head_response = self.s3_client.head_object(
                Bucket=self.bucket_name, Key=latest_key
            )

            # Скачиваем файл
            self.s3_client.download_file(self.bucket_name, latest_key, download_path)
            ...
            try:
                with open(download_path, "r", encoding="utf-8") as f:
                    data = json.load(f)
                    backup_info["users_count"] = (
                        len(data) if isinstance(data, dict) else 0
                    )
                    backup_info["content_preview"] = (
    ...
    def list_backups(self) -> list:
        ...
        try:
            response = self.s3_client.list_objects_v2(
                Bucket=self.bucket_name, Prefix="database_backups/"
            )
    ...
    def upload_folder_as_zip(self, folder_path: str, s3_key: str) -> dict:
        ...
        try:
            ...
            noFapLogger.info(
                f"Folder uploaded to S3: {s3_key} ({files_count} files, {zip_size} bytes)"
            )
            return upload_info
    ...
    def download_and_extract_zip(self, s3_key: str, extract_path: str) -> dict:
        ...
        try:
            ...
            noFapLogger.info(
                f"Archive downloaded and extracted: {s3_key} -> {extract_path} ({files_count} files)"
            )
...
def backup_all_to_s3():
    ...
    try:
        ...
        noFapLogger.info(
            "🎉 Complete backup (database + memes) to S3 completed successfully!"
        )
