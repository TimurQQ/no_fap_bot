...
async def ban_user(message: types.Message):
    ...
    noFapLogger.info(f"User {message.chat.username}({message.chat.id}) has tried to ban with comand {args}")
    if (len(args) == 0):
...
async def unban_user(message: types.Message):
    args = message.get_args()
    noFapLogger.info(f"User {message.chat.username}({message.chat.id}) has tried to unban with comand {args}")
    if (len(args) == 0):
        await message.answer(f"Command args passes incorrectly")
        return
    whom = args.split()[0].split("@")[-1]
...
async def no_admin_rights_handler(message: types.Message):
    noFapLogger.info(f"User {message.chat.username}({message.chat.id}) has tried to ban but he don't have rights")
...
async def get_black_list(message: types.Message):
    noFapLogger.info(f"User {message.chat.username}({message.chat.id}) has asked blacklist")
